...
|---|---|---|---|---|---|
| `寫 task` / `新增 task` / `加一張 task` | `JV-32/task-lifecycle:create`：先做 semantic intake；只有 `genuinely_new` 或核准的 `replace` 可建立 MC canonical task。 | `planning` 或 `project-init` | 比對相關 canonical tasks，提出含穩定 `id`、非空且同檔唯一 `order_label` 的草案，記錄四維 scope comparison、Vincent approval、`jv32_route` 與 `create` event；再執行 `node scripts/generate-data.mjs`。 | `npm run test:tasks` | MorroWise／control-plane task 有 id、order_label、title、status、track、done_condition、semantic intake、create evidence 與 `from_status: null`；order_label 缺少、空值、非字串或重複時 fail；`reuse` 不得寫入。 |
...
| `完成 task` / `mark task done` / `這個 task 完成` | `JV-32/task-lifecycle:complete` 加 `closeout-commit-routing`：只在 artifact evidence 與驗證存在後關閉。 | `worktree-commit` | 追加 complete event，更新 status、completed_at、commits、summary；可執行行為附 `test_contract` 與 Red／Green／regression `completion_evidence`，純文件／治理 task 則附 exemption reason、替代 verifier 與 evidence；再執行 `node scripts/generate-data.mjs`。若是 promoted subsystem version improvement，先完成 Admission Record review 與 architecture sync check。 | Task-specific verifier plus `npm run test:tasks` | complete event、closeout route、summary、`test_contract`、`completion_evidence` 與 durable artifact／verification 一致；runtime claim 另需安全真實 evidence，架構 version improvement 的 Admission Record 不得為 stale。 |
...
- 新增 MorroWise／control-plane task 的 `order_label` 是非空字串，且在該 canonical `tasks.json` 內唯一；canonical identity 仍是 `project/task-id`。
