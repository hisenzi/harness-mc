...
function compareOrderLabels(left, right) {
  const leftLabel = String(left.order_label || "");
  const rightLabel = String(right.order_label || "");
 ...
 */
export function sortTasksByPlan(tasks = [], { orderLabelAsSource = false } = {}) {
  if (orderLabelAsSource) {
    return [...tasks].sort(compareOrderLabels);
  }
