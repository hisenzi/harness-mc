...
const baseTasks = {
  tasks: [
    ...
    {
      id: "legacy-closed-task",
      title: "歷史已完成任務",
      status: "completed",
      track: "control-plane",
      order_label: "ACP-LEGACY-CLOSED",
      done_condition: "Fixture preserves a historical closed task created before the completion evidence contract.",
      ...
      task_lifecycle: {
        ...
        history: [{
          ...
          from_status: null,
          to_status: "completed",
          reason: "Fixture seeds a historical closed task without the newer completion contract.",
          evidence_refs: ["milestones/harness-mc/tasks.json"],
...
const historicalClosedAmendment = structuredClone(baseTasks);
const historicalClosedTask = historicalClosedAmendment.tasks.find((task) => task.id === "legacy-closed-task");
historicalClosedTask.summary = "Historical closed task metadata may be amended without retroactively fabricating Red or Green evidence.";
historicalClosedTask.task_lifecycle.history.push({
  operation: "amend",
  from_status: "completed",
  to_status: "completed",
  reason: "Fixture verifies closed-to-closed historical compatibility without adding a synthetic completion contract.",
  evidence_refs: ["milestones/harness-mc/tasks.json", "node scripts/verify-validate-tasks.mjs"],
  recorded_at: "2026-07-26"
});
writeJson(path.join(repo, "milestones", "harness-mc", "tasks.json"), historicalClosedAmendment);

const historicalClosedAmendmentPass = run(["--changed-only", "--project", "harness-mc"]);
if (!historicalClosedAmendmentPass.output.includes("Task validation OK")) {
  throw new Error("Expected a historical closed-to-closed amendment without synthetic completion evidence to pass.");
}
