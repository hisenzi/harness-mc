...
function verifyTaskLifecycleSchema(value) {
  ...
  const requiredEvidenceRule = value.allOf.find((rule) => rule.$comment === "required_completion_evidence_consistency");
  assert.ok(requiredEvidenceRule, "task lifecycle schema must conditionally require Red and Green completion evidence");
  assert.deepEqual(
    requiredEvidenceRule.then.properties.completion_evidence.required,
    ["red_evidence", "green_evidence"],
  );
  const runtimeEvidenceRule = value.allOf.find((rule) => rule.$comment === "runtime_completion_evidence_consistency");
  assert.ok(runtimeEvidenceRule, "task lifecycle schema must conditionally require real runtime evidence");
  assert.deepEqual(
    runtimeEvidenceRule.then.properties.completion_evidence.required,
    ["runtime_evidence"],
  );
